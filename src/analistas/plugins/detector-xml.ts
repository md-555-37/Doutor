...
export function detectarArquetipoXML(
  arquivos: string[],
    ...
    return [
      {
        ...
        dependencyMatches: [],
        filePatternMatches: xmlFiles.filter(
          (f) =>
            f.includes('web.xml') ||
            f.includes('sitemap') ||
            f.includes('rss') ||
        ...
        sugestaoPadronizacao:
          'Projeto web com XML detectado (RSS, SOAP, sitemap, etc)',
    ...
    return [
      {
        ...
        anomalias: [],
        sugestaoPadronizacao:
          'Projeto com arquivos XML de configuração detectado',
        explicacaoSimilaridade:
          'Múltiplos arquivos XML para configuração ou dados',
        descricao:
          'Projeto que usa XML para configuração ou armazenamento de dados',
        planoSugestao: {
          mover: [],
          conflitos: [],
          resumo: { total: 0, zonaVerde: xmlFiles.length, bloqueados: 0 },
