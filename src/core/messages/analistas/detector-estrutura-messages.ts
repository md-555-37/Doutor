...
export const DetectorEstruturaMessages = {
  ...
  muitosArquivosRaiz:
    'Muitos arquivos na raiz do projeto. Considere organizar em pastas.',
  ...
  projetoGrandeSemSrc:
    'Projeto grande sem pasta src/. Considere organizar o código fonte.',

  arquivosConfigDetectados: (detectados: string[]) =>
    `Arquivos de configuração detectados: ${detectados.join(', ')}`,

  multiplosEntrypointsAgrupados: ({
    previewGrupos,
    sufixoOcultos,
  }: EntrypointsAgrupadosArgs) =>
  ...
  multiplosEntrypointsLista: (preview: string[], resto: number) =>
    resto > 0
